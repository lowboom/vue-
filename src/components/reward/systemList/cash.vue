      ...
      <el-form ref="form" :model="form" label-width="80px" class="wrape">
        ...
        <div class="sendAmountWrap">
          <el-form-item label="数量限制">
            <el-radio-group v-model="sendAmount" @change="sendAmountShow">
              ...
              <el-radio class="radio" label="限制">限制
                <el-input v-model="sendAmountInput" max="10" v-if="sendAmountShowN"></el-input><span v-if="sendAmountShowN">个</span></el-radio>
            </el-radio-group>
          </el-form-item>
        </div>

          <el-form-item label="奖券面额">
            <el-radio-group v-model="faceFixed" @change="inputLimitShow">
              <el-radio class="radio" label="固定金额">固定金额</el-radio>
              <el-radio class="radio" label="非固定金额">非固定金额</el-radio>
            </el-radio-group>
          </el-form-item>

        <el-form-item label="奖券额度" v-if="inputLimitShowT">
          <el-input v-model="inputLimit" style="width:215px"></el-input>
        </el-form-item>
        <el-form-item label="奖券额度" v-if="inputLimitShowF">
        ...
        <el-form-item label="过期设置">
          <el-radio-group v-model="expireType" @change="changeShow">
            ...
            <el-radio class="radio"  label="永不过期">永不过期</el-radio>
            <div class="timeStartInput">
              <el-radio class="radio" label="有效期">有效期 <span class="timeStart" v-if="timestartShow1">自生成之日起</span><el-input v-model="input" max="10" v-if="timestartShow1"></el-input><span class="timeStartRight" v-if="timestartShow1">天</span></el-radio>
            </div>
            <div v-if="errorMe">
              <span>请填写天数</span>
            </div>
        ...
        <el-form-item label="发放时间">
          <el-radio-group v-model="provideTime" >
            <el-radio class="radio" label="投资后即时">投资后即时</el-radio>
            <el-radio class="radio" label="满标">满标</el-radio>
            <el-radio class="radio" label="放款">放款</el-radio>
          </el-radio-group>
        </el-form-item>
...
//      this.change()
  ...
  methods: {
    ...
    inputLimitShow: function(){
      if(this.faceFixed == '固定金额'){
        this.inputLimitShowT = true
        this.inputLimitShowF = false
      }else if(this.faceFixed == '非固定金额'){
        this.inputLimitShowF = true
        this.inputLimitShowT = false
      }
    ...
    subButton: function(){
      ...
      alert(this.inputLimit)


      if(this.faceFixed == 9){
        this.faceFixed = true
      }else if(this.faceFixed == 10){
        this.faceFixed = false
      }
      if(this.couponType == '' || this.inputName == '' || this.inputSource == '' || this.inputLimit == '' || this.inputNumber == '' || this.expireType =='' ){
        this.openPop();
        return
      }
      ...
      var formData = {
        ...
        "faceFixed":this.faceFixed,
      }

      $.ajax({
        type : "POST",
        contentType : "application/json",
        url : "http://192.168.1.130:8108/activity/createCoupon.do",
